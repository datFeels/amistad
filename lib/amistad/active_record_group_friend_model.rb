...
module Amistad
  module ActiveRecordGroupFriendModel
    ...
    included do
      ...
      has_many  :pending_invited,
        -> { where("friendships.pending IS TRUE AND friendships.blocker_id IS NULL")},
        :through => :friendships,
        :source => :friend

      has_many  :invited,
        -> { where('friendships.pending IS FALSE AND friendships.blocker_id IS NULL')},
        :through => :friendships,
        :source => :friend
      ...
      has_many  :inverse_friendships,
        :class_name => "Amistad::Friendships::#{Amistad.group_friendship_model}",
    ...
    def invite(user)
      ...
      Amistad.group_friendship_class.new{ |f| f.friendable = self ; f.friend = user }.save
    end
