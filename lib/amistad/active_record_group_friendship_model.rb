  ...
  module ActiveRecordGroupFriendshipModel
    ...
    included do
      belongs_to :friendable,
        :class_name => Amistad.group_friend_model,
    ...
    def can_block?(friendable)
      active? && (approved? || (pending? && self.friend_id == friendable.id && friendable.class.to_s == Amistad.group_friend_model))
    end

    # returns true if a friendship can be unblocked by given friendable
    def can_unblock?(friendable)
      blocked? && self.blocker_id == friendable.id && friendable.class.to_s == Amistad.group_friend_model
    end
  end
end
